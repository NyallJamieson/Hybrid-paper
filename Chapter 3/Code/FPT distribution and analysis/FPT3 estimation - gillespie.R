...
L3 <- out1[length(which(times<=T3)),2]
M3 <- out1[length(which(times<=T3)),3]
...
MODEL.simul <- function(params,maxstep=10000000,count=0){
  ...
  while (count<1){
    ...
    output[1,] <- x
    k <- 1
    while ((k <= maxstep) && (x["L"] > 0 | x["M"] > 0) && ((x["L"] < L3) || (x["M"] < M3))){
      k <- k+1
...
results_gillespie <- as.data.frame(na.omit(do.call(rbind,parallel::mclapply(1:10000,MODEL.simul,mc.cores=8)),na.action="omit"))

write.csv(results_gillespie,"T3_gill.csv")
