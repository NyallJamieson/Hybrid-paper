...
colnames(results_deterministic) <- c("lambda","eps","T2","L","M")
...
while (tally < 1000){
  ...
  if (error_check==0){
    tryCatch({
      # Code that might throw an error
      results_deterministic <- dplyr::bind_rows(results_deterministic,data.frame(lambda=lambda,eps=E,T2=T,L=L2,M=M2))
...
write.csv(results_deterministic,"~/PhD-work/Chapter 3/Data/Sensitivity analysis/Deterministic/sens_T2_lambda_deterministic.csv")
...
stoch <- function(y){
  ...
  h <- results_deterministic[y,1]
  L2 <- results_deterministic[y,4]
  M2 <- results_deterministic[y,5]
  
  frame <- data.frame("lambda" = rep(h, stoch_sample_size), "time" = NA)
...
saveRDS(results_stochastic,"~/PhD-work/Chapter 3/Data/Sensitivity analysis/Stochastic/sens_T2_lambda_stochastic.rds")
