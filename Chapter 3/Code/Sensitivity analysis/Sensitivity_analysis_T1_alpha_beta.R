...
colnames(results_deterministic) <- c("alpha","beta","eps","T1","L","M")
...
while (tally < 1000){
  ...
  results_deterministic <- dplyr::bind_rows(results_deterministic,data.frame(alpha=a,beta=b,eps=E,T1=T,L=L2,M=M2))
  tally <- tally + 1
}

write.csv(results_deterministic,"~/PhD-work/Chapter 3/Data/Sensitivity analysis/Deterministic/Sens_T1_alphabeta_deterministic.csv")
