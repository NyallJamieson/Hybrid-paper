...
colnames(results_deterministic) <- c("alpha","beta","eps","T2","L","M")
...
while (tally < 1000){
  ...
  if (error_check==0){
    tryCatch({
      # Code that might throw an error
      results_deterministic <- dplyr::bind_rows(results_deterministic,data.frame(alpha=a,beta=b,eps=E,T2=T,L=L2,M=M2))
...
write.csv(results_deterministic,"sens_T2_alphabeta_deterministic.csv")
