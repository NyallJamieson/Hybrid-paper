...
plot(eps,times,col="red",lwd=2,type="l",xlab=expression(epsilon[4]),ylab=expression(T[4]),main="Threshold time for COV condition")
grad <- (times[length(times)]-times[1])/(eps[length(eps)]-eps[1])
...
eps[t]
times[t]
# for T4 we have: eps_{4}=0.01093, T_{4}=49.9
...
# Produce plot
plot(out1[,1],sd_over_mean,xlab="Time in hours",ylab=expression(c[BP]),main="Coefficient of variation for MTBP model",col="red",type="l",lwd=2)
