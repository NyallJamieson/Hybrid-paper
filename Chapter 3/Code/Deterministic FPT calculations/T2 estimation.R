...
#plot(out1[,1],sd_over_mean_bp/frac,lwd=2,col="red",type="l",xlab="Time in hours",ylab=expression(c[BD]/c[SDE]),main="Ratio of COV for BP and SDE models",ylim=c(8,10.5))

#plot(out1[251:1501,1],abs(F-m),col="blue",type="l")
#length(which(abs(F-m)<0.0001))
#which <- out1[1501-length(which(abs(F-m)<0.0001)),1]
...
plot(eps,CTimes,col="red",lwd=2,type="l",xlab=expression(epsilon[2]),ylab=expression(T[2]),main="Threshold time for COV condition")
grad <- (CTimes[length(CTimes)]-CTimes[1])/(eps[length(eps)]-eps[1])
lines(c(eps[1],eps[length(eps)]),c(CTimes[1],CTimes[length(CTimes)]),type="l",lty=2)
lines(seq(0,2*eps[t2],by=0.00000001),CTimes[t2]-grad*(eps[t2]-seq(0,2*eps[t2],by=0.00000001)),lty=2)
symbols(eps[t2],CTimes[t2],circles=1,add=TRUE,inches=0.1)
