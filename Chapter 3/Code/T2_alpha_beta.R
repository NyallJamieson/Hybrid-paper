...
while (tally < 25){
  ...
  a <- unname(gamma_pi[1]*gamma_pi[2])
  b <- unname(gamma_pi[1]*(1-gamma_pi[2]))
  ...
  results <- parallel::mclapply(1:1000,trajectory, mc.cores = 12)
  ...
  for (i in 1:length(eps)){
    CTimes <- append(CTimes,t[length(which(F>q+eps[i]))+250])}
...
write.csv(results_deterministic,"sens_T2_deterministic_alpha_beta.csv")
...
results_stochastic <- dplyr::bind_rows(results_stochastic,as.data.frame(na.omit(do.call(rbind,parallel::mclapply(1:1000,stoch,mc.cores=80)),na.action="omit")))

write.csv(results_stochastic,"sens_T2_stochastic_alpha_beta.csv")
